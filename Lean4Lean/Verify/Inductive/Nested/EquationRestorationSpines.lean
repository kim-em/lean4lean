...
theorem NestedRestorationPlan.AtomicProvenance.bvarSpineAlignment
  ...
  | @hit source target hhit =>
      have hhead : source.bvarHead? = some index := by
        ...
        rw [hspine]
      rw [Hatomic.restoreNode_eq_none_of_bvarHead hhead] at hhit
      cases hhit
  | @bvar i hnone =>
  ...
  | @const name levels hnone =>
      ...
      cases hspine
  | @app sourceFn sourceArg targetFn targetArg hnone hfn harg ihfn iharg =>
      ...
      | mk sourceHead priorSourceArgs =>
        ...
        · rw [← hargs]
          exact Lean4Lean.VerifyInductive.List.Forall₂.append' Haligned
            (.cons harg .nil)
  | @lam sourceDomain sourceBody targetDomain targetBody hnone hdomain hbody
      ihdomain ihbody =>
      change (VExpr.lam sourceDomain sourceBody, []) =
        (.bvar index, sourceArgs) at hspine
      cases hspine
  | @forallE sourceDomain sourceBody targetDomain targetBody hnone hdomain hbody
      ihdomain ihbody =>
